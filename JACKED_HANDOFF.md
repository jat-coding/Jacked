...
- **Tap-to-clear (2026-09-24):** focusing any `input.si` (set weight/reps/duration/distance, live workout + history editor) empties it and shows the old value as the placeholder; blur with nothing typed restores it (state too). Delegated `focusin`/`focusout` handlers near the keyboard-handling block.
- **Browser tests live in `tests/e2e/`** (not /tmp): `PW=<path to node_modules/playwright> node tests/e2e/run.mjs`. Serves `jacked-pwa/` locally, blocks Supabase, fixes the clock/timezone.
