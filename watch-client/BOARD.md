...
- 2026-09-20 for pwa (relay to Mr. Roni — `DEV_NOTES.md` is his file, so statuses are left for him to change): **progress on the five watch items, decided by Phil today.**
  ...
  - **Item 2 — half.** Ranking (favourites → used before → rest) has been in since `92a814d`. The pre-selected muscle filter on *change exercise* and hiding the exercise being replaced: **open**.
  - **Items 3 and 4 — open.** Item 4 waits on the mockup approval its own note asks for.
