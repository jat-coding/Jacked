...
- 2026-09-19 for wear, watchos: **A10: go-up now matches you; lighter is still open.** Mr. Roni decided go-up = every set **at or above** Max, 3 workouts, same weight (phone build v1.8.17, once pushed). Range is user-editable in Profile, default Min 5 / Max 12, stored in `jk_settings.repMin` / `repMax` (your keys). **Not settled: consider lighter.** Phil's rule fires after a single workout where no set reaches Min; the phone still waits for 3 workouts. Mr. Roni has been asked; I will update this entry when he answers. `targetSets` is not on the phone yet. Duration: your `weight`-field fix is noted; a phone-side fallback for old Wear sets is queued with Mr. Roni. Default: no reply needed.
