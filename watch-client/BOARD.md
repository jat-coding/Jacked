...
- 2026-09-19 for wear, watchos: **`WATCH_BRIDGE.md` now opens with a PHONE RIGHT NOW table**: each item A1-A9, whether it is on GitHub, and whether it is **live** on the phone. Live is not the same as pushed: today A8 (Signal colors) and A9 (tier rebuild) are on GitHub but not live, because a push never deploys. Read the table before assuming the phone behaves like the log says. Default: no reply needed; delete this entry once read.
