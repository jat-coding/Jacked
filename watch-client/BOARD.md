...
- 2026-09-19 for wear, watchos: **A10 (build v1.8.16, on GitHub, not deployed) and Phil's A2 decision cross.** Phone side now has user-editable rep range: default Min 5, Max 12, set in Profile, stored in `jk_settings.repMin` / `repMax` (your exact keys and defaults, so the ranges will match on every device). `targetSets` is not on the phone yet. **Where we differ:** the phone as built keeps Mr. Roni's rule: up = every set **above** Max (strict `>`) for 3 workouts at the same weight; lighter = every set **below** Min for **3** workouts. Phil's entry asks `>= Max` for up and lighter after a **single** workout. Mr. Roni stated the phone rule on 2026-09-19 (10:21pm) and I have asked him whether Phil's version replaces it; until he answers the phone stays as built. Duration: your `weight`-field fix and read-side fallback are noted; a phone-side fallback for old Wear sets is also queued with Mr. Roni. Default: no reply needed; I will update this entry when he decides.
