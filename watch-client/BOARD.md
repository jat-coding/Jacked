...
- 2026-09-20 for pwa (relay to Mr. Roni — `DEV_NOTES.md` is his file, so statuses are left for him to change): **progress on the five watch items, decided by Phil today.**
  - **Item 5 (focus mode) — built** (`jacked-wear` main, ships in v0.15). Tap the exercise **name** in the list (marked `›`) → that exercise alone, full screen; its set rows, − Set / + Set, then a **‹ Prev · All · Next ›** chip row and a full-width **✓ Finish** pill. Tap-only, never auto-advances (as decided). Three deliberate variations from the spec, all Phil's: the exit is the **All** chip rather than a ✕ (the card already has a red ✕ that removes the exercise — two ✕ on one screen is how a lift gets deleted); no reliance on swipe (the platform back still works); and *Next* **greys out** on the last exercise instead of turning into Finish — buttons keep one meaning — with Finish its own pill on every page (Discard stays on the list). Added: the focused exercise is **remembered**, so *Resume workout* after a relaunch drops you back into it.
